...
int main(int argc, char **argv)
{
        ...
        /* общедоступный имен. канал (создается перед запуском сервера)
        открывается на запись и в него записываются имена личного канала и требуемого файла */
        ...
        /* личный имен. канал открывается для чтения */
        ...
        // личный имен. канал закрывается и удаляется из текущего каталога
        close (fdpriv);
        unlink (msg.privfifo);
        exit(0);
}
