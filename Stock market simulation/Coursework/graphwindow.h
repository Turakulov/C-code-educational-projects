...
public:
    explicit GraphWindow( QWidget *parent = nullptr,
                          graphwindow_t type = HOLDERS_WINDOW);
    ~GraphWindow();

    void drawCompanyGraphs(int days, const QList<QList<int>> &data,
                           const QStringList &names);

    void drawHolderGraphs(int days, const QList<QList<int>> &holders,
                          const QList<QList<int>> &holdersWithAnalysis,
                          const QStringList &holdersNames,
                          const QStringList &holdersWithAnalysisNames);
